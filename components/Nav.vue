<template>
    <div class="flex justify-between p-[2rem] items-center gap-[1rem]">
        <div>
            <img src="https://xitcoin.org/wp-content/uploads/2024/12/logo-dark.png" class="w-[100px] h-[50px]" alt="">
        </div>
        <div>
            <button
                v-if="!account"
                class="bg-[#2A85F3] hover:bg-[#FB8D00] text-white  py-2 px-4 rounded inline-flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" width="1.5rem" height="1.5rem" viewBox="0 0 212 189"
                    id="metamask">
                    <g fill="none" fill-rule="evenodd">
                        ...
                        <polygon fill="#CDBDB2"
                            ...
                            transform="matrix(-1 0 0 1 256.5 0)"></polygon>
                        <polygon fill="#393939"
                            points="90.563 152.438 88.313 171 91.125 168.75 120.375 168.75 123.75 171 121.5 152.438 117 149.625 94.5 150.188">
                        </polygon>
                        <polygon fill="#F89C35"
                            points="75.375 27 88.875 58.5 95.063 150.188 117 150.188 123.75 58.5 136.125 27"></polygon>
                        <polygon fill="#F89D35"
                        ...
                        <polygon fill="#DFCEC3"
                            points="60.75 173.25 90.563 152.438 88.313 170.438 88.313 180.563 68.063 176.625"></polygon>
                        <polygon fill="#DFCEC3"
                            points="121.5 173.25 150.75 152.438 148.5 170.438 148.5 180.563 128.25 176.625"
                            transform="matrix(-1 0 0 1 272.25 0)"></polygon>
                        <polygon fill="#393939" points="70.313 112.5 64.125 125.438 86.063 119.813"
                            transform="matrix(-1 0 0 1 150.188 0)"></polygon>
                        ...
                        <g transform="matrix(-1 0 0 1 211.5 0)">
                            ...
                            <polygon fill="#EA8D3A" points="46.125 101.813 65.25 119.813 65.25 137.813"></polygon>
                            <polygon fill="#F89D35"
                                points="65.25 120.375 87.75 126 95.063 150.188 90 153 65.25 138.375"></polygon>
                ...
                <span class="pl-[0.5rem]" @click="connect">Connect Wallet</span>

            </button>
            <button
                v-else
                class="bg-transparent hover:bg-black-400 text-white-800 font-bold py-2 px-4 rounded inline-flex items-center max-w-[8rem] overflow-hidden">
               
                <span class="pl-[0.5rem] truncate text-[#54595f]">{{ account }}</span>
...
const connect =async()=>{
    ...
    //   console.log(await web3.eth.getAccounts())
      ...
      account.value= await web3.eth.getAccounts()[0]
    }
  } catch (error) {
...
onMounted(async()=>{
     ...
     window.ethereum.on('accountsChanged', function (accounts) {
        // Handle the change in accounts here
